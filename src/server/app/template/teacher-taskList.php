...
list($res, $data) = (new TaskModule())->getTaskList($args);
...
<body>
    <div class="container">
        ...
        <div class="d-flex justify-content-end">
            <a href="task.php?id=0" class="btn btn-success">
        ...
        ?>
            <div class="alert alert-info rounded-0 my-3" role="alert">
                <div>
                    <p class="m-0">Не найдена ни одна задача.</p>
                </div>
            </div>
        <?php
        }
        ...
        window.addEventListener('DOMContentLoaded', function() {
            for (const item of document.querySelectorAll('button[data-action="remove"]')) {
                item.addEventListener('click', (e) => {
                    e.stopPropagation();
                    window.location.assign(`task.php?id=${item.dataset.id}&action=remove`);
                });
            }
            ...
            for (const item of document.querySelectorAll('.table.clickable-rows>tbody>tr')) {
                item.addEventListener('click', (e) => {
                    e.stopPropagation();
                    window.location.assign(`task.php?id=${item.dataset.id}`);
                });
            }
        });
    </script>

...
export default class SerieFormRemove {
    _el = {};
    _atm = {};
    ...
    constructor(settings = {}) {
        ...
        this._atm.nameInput = <Input className="col-12" label={i18n(langId, 'TTL_SERIE_NAME')} value={serie.name} disabled={true} />;
    ...
    _onRemoveButtonClick = () => {
        ...
        this._callRemoveSerie({ id: serieId });
    };
    ...
    _callRemoveSerie = async (payload) => {
        this._beforeCallRemoveSerie();
        try {
            const resp = await fetcher('removeSerie', payload);

            if (resp.status === 'ok') {
                openSiteURL('serie-list.php');
            }

            this._afterCallRemoveSerie({ status: resp.status, data: resp.data });
        } catch (e) {
            debugger;
            this._afterCallRemoveSerie({ status: 'error' });
        }
    };
